...
def main(args):
    ...
    return vis_acc, clu_acc, runtime
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--data_name', type=str, default='mnist-test')
    parser.add_argument('--n_cluster', type=int, default=2)
    parser.add_argument('--pretrain', type=int, default=0)

    # model param
    parser.add_argument('--perplexity', type=int, default=3)   #3
    ...
    # train param
    parser.add_argument('--batch_size', type=int, default=50)  #500
    ...
    vis_acc_values, clu_acc_values, times = [], [], []
    for i in range(5):
        args['data_name'] = f'two planes{i+1}'

        vis_acc, clu_acc, runtime = main(args)
        vis_acc_values.append(vis_acc)
        clu_acc_values.append(clu_acc)
        times.append(runtime)

    vis_acc_mean = np.mean(vis_acc_values)
    vis_acc_std = np.std(vis_acc_values)
    clu_acc_mean = np.mean(clu_acc_values)
    clu_acc_std = np.std(clu_acc_values)
    times_mean = np.mean(times)

    print('Result: {}, {}, {}, {}'.format(vis_acc_mean, vis_acc_std, clu_acc_mean, clu_acc_std))
    print(f'time: {times_mean}')
