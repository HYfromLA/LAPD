function accuracy = accuracy(true_labels, predicted_labels)
    % This function calculates clustering accuracy using the Hungarian algorithm,
    % handling cases where labels do not match.

    % Normalize labels to start from 1
    [~, ~, normalized_true_labels] = unique(true_labels);
    [~, ~, normalized_predicted_labels] = unique(predicted_labels);

    % Number of unique clusters
    K = max(max(normalized_true_labels), max(normalized_predicted_labels));

    % Create contingency matrix
    contingency = zeros(K, K);
    for i = 1:length(normalized_true_labels)
        contingency(normalized_true_labels(i), normalized_predicted_labels(i)) = ...
            contingency(normalized_true_labels(i), normalized_predicted_labels(i)) + 1;
    end

    % Use munkres to find optimal assignment (requires munkres.m)
    %addpath('path_to_munkres'); % Replace with actual path to munkres.m
    [assignment, ~] = munkres(-contingency);

    % Calculate the optimal accuracy
    matched_counts = sum(contingency(sub2ind(size(contingency), (1:K)', assignment')));
